...
router.get("/", async (req, res) => {
  try {
    ...
    const analyticsTransactions = await Transaction.find({
      ...userQuery,
      ...dateFilter,
    });
    ...
    while (iterDate <= endDate) {
      dayMap.set(iterDate.toISOString().split("T")[0], {
        income: 0,
        expense: 0,
      });
    ...
    const currentMonthName = new Date(year, month - 1).toLocaleString(
      "default",
      { month: "long" }
    );
  ...
  try {
    ...
    if (newTransaction.isRecurring) {
      processRecurringTransactions().catch((err) =>
        console.error("Error processing recurring transactions on-demand:", err)
      );
    }
    ...
    if (type === "expense") {
      ...
      if (budget) {
        ...
        if (totalSpent >= budget.amount * 0.9) {
          ...
          req.flash(
            "warning_msg",
            `Warning: You've spent ${percentageSpent}% of your ${category} budget`
          );
...
router.delete("/all-by-month", async (req, res) => {
  try {
    ...
    req.flash(
      "success_msg",
      `All transactions for ${new Date(year, month - 1).toLocaleString(
        "default",
        { month: "long" }

...
class BudgetRLService {
  ...
  static getPaceRecommendation(category, paceData) {
    ...
    if (pacePercentage > 100) {
      ...
      if (remaining === 0) {
        ...
        // Already overspent
        message = `You've exceeded your limit of your ${category} budget. STOP spending or increase the budget.`;
      } else {
        // Projected to overspend
        const overBudgetAmount = projectedSpending - budgetAmount;
        message = `You've spent ${formatCurrency(
          spent
        )}. At this pace, you are projected to go over budget by ${formatCurrency(
          overBudgetAmount
        )}.`;
      }
    }
    ...
    // Caution - On track but close to limit (90%-100% of budget used)
      ...
      const daysUntilExceeded =
        dailyRate > 0 ? Math.ceil(remaining / dailyRate) : Infinity;
      message = `You've spent ${formatCurrency(
        spent
      )} of your ${category} budget. At this pace, you'll exceed your limit in ${daysUntilExceeded} days. CONTROL your spending.`;
    }
    ...
    // Positive - Well under budget (≤60% of budget used)
      ...
      message = `Only ${formatCurrency(
        spent
      )} of your ${category} budget used and ${daysLeft} days left. Strong financial control.`;
    }
    ...
    // Info - On track (61%-89% of budget used)
      ...
      message = `You've used ${formatCurrency(
        spent
      )}, with ${daysLeft} days remaining. You're on track.`;
    }
  ...
  static async generateRecommendations(userId, budgets, transactions) {
    ...
    for (const budget of budgets) {
      ...
      if (budget.month === currentMonth && budget.year === currentYear) {
        // Current month logic
        console.log(
          `Processing current month budget for category: ${budget.category}`
        );

        if (spent === 0) {
          const noSpendingMessage = `No spending recorded for ${
            budget.category
          } yet. Your full budget of ${formatCurrency(
            budget.amount
          )} is available.`;
        ...
        const paceData = this.calculateBudgetPace(
          budget,
          spent,
          currentDay,
          daysInMonth
        ...
        const recommendation = this.getPaceRecommendation(
          budget.category,
          paceData
        );
        ...
        if (budgetDate < todayDateStartOfMonth) {
          // Past month
          message = `For this past month, you spent ${formatCurrency(
            spent
          )} of your ${formatCurrency(budget.amount)} budget.`;
        } else {
          // Future month
          message = `You have a budget of ${formatCurrency(
            budget.amount
          )} set for a future month.`;
        }
    ...
    if (
      ...
      if (overallPace > 110) {
        overallMessage =
          "Looking at the big picture, you might want to slow down your spending a bit this month.";
        overallType = "warning";
      } else if (overallPace < 90) {
        overallMessage =
          "Overall, you're doing great with your finances this month! Nice work saving money.";
        overallType = "positive";
      } else {
        overallMessage =
          "Your overall spending is perfectly on track this month. You're balancing things well!";
        overallType = "positive";
      }
    ...
    if (recommendations.length < 3) {
      recommendations.push({
        message:
          "Having clear savings goals can help keep you motivated with your budget.",
        type: "general",
        category: "general",
      });
      recommendations.push({
        message:
          "Many people find the 50/30/20 approach helpful: 50% for needs, 30% for wants, and 20% for savings.",
        type: "general",
        category: "general",
      });
    }
  ...
  static async saveRecommendations(userId, recommendations) {
    try {
      ...
      const recentHistory = model.recommendationHistory.filter(
        (rec) => new Date(rec.date).getTime() > thirtyDaysAgo
      );
        ...
        if (!budgetAmount || budgetAmount <= 0) {
          ...
          if (budgetYear !== currentYear || budgetMonth !== currentMonth) {
            ...
            if (budgetDate < todayDate) {
              message = `For this past month, you spent ${BudgetRLService.formatCurrency(
                spent
              )} of your ${BudgetRLService.formatCurrency(
                budgetAmount
              )} budget.`;
            } else {
              message = `You have a budget of ${BudgetRLService.formatCurrency(
                budgetAmount
              )} set for a future month.`;
            }
            ...
            const message = `You have spent ${BudgetRLService.formatCurrency(
              spent
            )} of your ${BudgetRLService.formatCurrency(
              budgetAmount
            )} budget (${percentageSpent.toFixed(2)}%).`;
